...
static bool write_bin_u8(const string& path, const uint8_t* data, size_t n) {
    ...
    if (!f) { cerr << "[debug] ERROR: cannot open " << path << "\n"; return false; }
    fwrite(data, 1, n, f);
    fclose(f);
    cout << "[debug] bin → " << path << " (" << n / 1e6 << " MB)\n";
    return true;
}
...
static bool write_png(const string& path, int rows, int cols, const uint8_t* rgb) {
    FILE* f = fopen(path.c_str(), "wb");
    if (!f) { cerr << "[debug] ERROR: cannot open " << path << " for write\n"; return false; }

    png_structp png  = png_create_write_struct(PNG_LIBPNG_VER_STRING, nullptr, nullptr, nullptr);
    if (!png) { cerr << "[debug] ERROR: png_create_write_struct failed\n"; fclose(f); return false; }

    png_infop info = png_create_info_struct(png);
    if (!info) {
        cerr << "[debug] ERROR: png_create_info_struct failed\n";
        png_destroy_write_struct(&png, nullptr); fclose(f); return false;
    }
    if (setjmp(png_jmpbuf(png))) {
        cerr << "[debug] ERROR: libpng error writing " << path << "\n";
...
static uint8_t* rasterize_layer(const string& shp_path,
    ...
    if (!shp_ds) { cerr << "[debug] ERROR: cannot open " << shp_path << "\n"; return nullptr; }

    OGRLayer* layer = shp_ds->GetLayer(0);
    if (!layer) {
        cerr << "[debug] ERROR: no layer in " << shp_path << "\n";
    ...
    if (geoms.empty()) { cerr << "[debug] WARN: no geometries in " << shp_path << "\n"; return nullptr; }
    cout << "[debug] loaded " << geoms.size() << " geoms\n";
    cout << "[debug] MEM raster: " << COLS << " x " << ROWS
         << " origin=(" << ox << ", " << oy << ")\n";
    ...
    if (err != CE_None) {
        cerr << "[debug] ERROR: GDALRasterizeGeometries failed (CPLErr=" << err << ")\n";
    ...
    if (rio != CE_None)
        cerr << "[debug] WARN: RasterIO returned error " << rio << "\n";

    // quick scan — how many cells burned?
    size_t burned = 0;
    for (size_t i = 0; i < n; i++) if (buf[i]) burned++;
    cout << "[debug] burned cells after rasterize: " << burned << "\n";
...
static void expand_buffer(uint8_t* buf, size_t N, int ROWS, int COLS, int radius) {
    if (radius <= 0) {
        cout << "[debug] radius=0 — skipping expansion\n";
        return;
    }
    ...
    cout << "[debug] expanded → " << expanded << " cells\n";
}
...
static void print_elev_stats(const ElevCell* cells, size_t N) {
    ...
    cout << "[debug] elevation : " << emin << "m → " << emax << "m\n";
    cout << "[debug] slope     : " << smin << "° → " << smax << "°\n";
    cout << "[debug] nodata    : " << nodata << " (" << (nodata * 100.0 / N) << "%)\n";
}
...
int main() {
    ...
    cout << "[debug] Grid       : " << ROWS << " x " << COLS
         << " (" << N / 1e6 << "M cells)\n";
    cout << "[debug] origin     : (" << ox << ", " << oy << ")\n";
    cout << "[debug] dist_road  : " << dist_road  << "m (" << dist_road  / RESOLUTION << " cells)\n";
    cout << "[debug] dist_rail  : " << dist_rail  << "m (" << dist_rail  / RESOLUTION << " cells)\n";
    cout << "[debug] dist_water : " << dist_water << "m (" << dist_water / RESOLUTION << " cells)\n";
    cout << "[debug] dist_ib    : " << dist_ib    << "m (" << dist_ib    / RESOLUTION << " cells)\n";
    cout << "[debug] elev grid  : " << ELEV_C << " x " << ELEV_R << "\n\n";
    ...
    for (auto& job : jobs) {
        ...
        if (!buf) { cerr << "[debug] SKIP: " << job.name << "\n"; continue; }

        size_t raw = 0;
        for (size_t i = 0; i < N; i++) if (buf[i]) raw++;
        cout << "[debug] raw hits   : " << raw
             << " (" << (raw * 100.0 / N) << "%)\n";

        if (raw == 0)
            cerr << "[debug] WARN: no cells burned — geotransform or SHP issue\n";

        expand_buffer(buf, N, ROWS, COLS, job.distance / RESOLUTION);

        size_t final_hits = 0;
        for (size_t i = 0; i < N; i++) if (buf[i]) final_hits++;
        cout << "[debug] final hits : " << final_hits
             << " (" << (final_hits * 100.0 / N) << "%)\n";
    ...
    // ── elevation + slope ──────────────────────────────────────────────────
    cout << "\n=== elevation + slope ===\n";
    cout << "[debug] reading " << ELEV_C << " x " << ELEV_R
    ...
    if (!cells) { cerr << "[debug] ERROR: cannot allocate ElevCell buffer\n"; return 1; }

    FILE* ef = fopen(elev_bin_path.c_str(), "rb");
    if (!ef) {
        cerr << "[debug] WARN: " << elev_bin_path << " not found — skipping\n";
        free(cells);
    } else {
        ...
        cout << "[debug] elev.bin size  : " << file_bytes / 1e6 << " MB\n";
        cout << "[debug] expected size  : " << expected_bytes / 1e6 << " MB\n";

        if (file_bytes != expected_bytes) {
            cerr << "[debug] ERROR: size mismatch — "
        ...
        write_bin_elevcell("bin/elevation.bin", cells, read);

        uint8_t* rgb_e = elev_to_rgb(cells, read);
        if (rgb_e) {
            write_png("image/elevation.png", ELEV_R, ELEV_C, rgb_e);
            free(rgb_e);
        }
